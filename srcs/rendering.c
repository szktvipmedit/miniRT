...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/22 14:50:38 by kousuzuk          #+#    #+#             */
/*   Updated: 2023/12/22 14:50:57 by kousuzuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_set_basis_vectors(t_ray *ray, t_camera camera)
{
	...
	ray->y_dir = v_cross(camera.forward_dir, ray->x_dir);
	if (camera.forward_dir.x == 0 && camera.forward_dir.y != 0
		&& camera.forward_dir.z == 0)
	{
		if (camera.forward_dir.y > 0)
		{
...
static void	ft_set_eye_ray(t_ray *ray, t_camera camera, int x, int y)
{
	...
	ray->pw = v_add(v_add(camera.camera_position, v_mult(camera.forward_dir,
					screen_distance)), v_add(v_mult(ray->x_dir, dx),
				v_mult(ray->y_dir, dy)));
...
void	ft_rendering(t_rt *rt)
{
	...
	while (y < HEIGHT)
	{
		...
		while (x < WIDTH)
		{
			rt->color = ft_init_color3(0.0, 0.0, 0.0);
